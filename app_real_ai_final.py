...
def generate_image():
    ...
    try:
        ...
        # Try to generate real AI image first
        image = generate_real_ai_image(prompt)
        
        if image is None:
            # Fallback to beautiful placeholder
            print("Real AI generation failed, using beautiful placeholder")
...
def generate_real_ai_image(prompt):
    """Generate real AI image using Hugging Face API"""
    try:
        headers = {}
        if HF_TOKEN:
...
def health_check():
    ...
    status = "Real AI Ready" if HF_TOKEN else "Beautiful Placeholder Mode"
    return jsonify({
        'status': 'healthy',
        'ai_mode': status,
        'note': 'Tries real AI first, falls back to beautiful placeholders'
    })

if __name__ == '__main__':
    print("🎨 Starting Real AI Text-to-Image Generation Application...")
    if HF_TOKEN:
